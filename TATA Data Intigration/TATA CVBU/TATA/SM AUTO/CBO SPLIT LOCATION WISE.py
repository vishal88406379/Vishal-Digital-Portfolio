...
def process_files():
    try:
        ...
        # Handle NaN and apply string operations correctly
        df['PartyName'] = df['PartyName'].fillna('').astype(str)
        df = df[~df['PartyName'].str.startswith('CSM')]
        df = df[~df['PartyName'].isin(['Tata Motors Ltd.'])]

        df['External Reference'] = df['External Reference'].fillna('').astype(str)
        df = df[~df['External Reference'].str.strip().str.endswith(('(E)', '(V', '(V)'))]

        # Convert quantity columns to numeric
        for col in ['Requested Quantity', 'Confirmed Quantity', 
                    'Fulfilled Quantity', 'Invoiced Quantity', 'Pending Qty']:
        ...
        # Ensure all main output columns exist
